...
describe("GET /employees at scale", () => {
  it("reports the exact filtered total and page count", async () => {
    const res = await request(app).get("/employees").query({ pageSize: 100, page: 1 });
  ...
  it("returns a full final page on a deep page request", async () => {
    const res = await request(app).get("/employees").query({ pageSize: 100, page: 30 });
  ...
  it("keeps the sort order stable across page boundaries", async () => {
    const p1 = await request(app).get("/employees").query({ sort: "salaryAmount", order: "desc", pageSize: 50, page: 1 });
    const p2 = await request(app).get("/employees").query({ sort: "salaryAmount", order: "desc", pageSize: 50, page: 2 });
